...
def train_gnn(model, optimizer, train_loader):
    ...
    for i, batch in enumerate(train_loader):
        ...
        '''
        emb32, output = model(embeddings[batch.subset].cuda(), text_embeddings.cuda(), edge_index.cuda())
...
def main_gnn(model, optimizer):
    ...
    for epoch in range(args.epochs):
        ...
        if f1_macro > best:
            torch.save(model.state_dict(), args.path + 'gnn1.pth')
            best = f1_macro

    state_dict = torch.load(args.path + 'gnn1.pth')
...
for i in range(5):
    ...
    for j in range(1):
        # 4096 384
        ##gnn = BWGNN(384, args.hidden, 2).cuda()
        gnn = GeniePathLazy(384, 2, 'cuda').cuda()
